// components/modules/(dashboard)/(common)/profile/platformSuperAdminProfile/PlatformSuperAdminProfile.tsx
"use client";
...
const PlatformSuperAdminProfile = ({ userData }: { userData: IUser }) => {
  ...
  const [formData, setFormData] = useState({
    name: userData.name || "",
    phone: userData.platformSuperAdmin?.phone || "",
  });
  ...
  return (
    <Card className="w-full max-w-3xl mx-auto">
      <CardHeader className="flex flex-row items-center justify-between bg-linear-to-r from-purple-500/10 to-blue-500/10 rounded-t-lg">
        <div>
          <CardTitle className="text-2xl font-bold flex items-center gap-2">
            <Crown className="w-6 h-6 text-purple-500" />
            {isEditing ? "Edit Profile" : "Platform Super Admin Profile"}
          </CardTitle>
        ...
        <form onSubmit={handleSubmit} className="space-y-6">
          ...
          <div className="flex flex-col items-center">
            {isEditing ? (
              ...
              <div className="relative h-24 w-24 rounded-full overflow-hidden border-4 border-purple-500/20">
                <Image
...
export default PlatformSuperAdminProfile;