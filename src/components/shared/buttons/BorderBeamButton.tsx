...
const BorderBeamButton = ({ children }: { children: React.ReactNode }) => {
  return (
    <Button size={"sm"} className="relative inline-flex h-12 overflow-hidden rounded-full p-0.5 focus:outline-none focus:ring-2 focus:ring-slate-400 focus:ring-offset-2 focus:ring-offset-slate-50">
      ...
      <span className="inline-flex h-full w-full cursor-pointer items-center justify-center rounded-full bg-secondary px-5 py-1 text-sm font-medium text-primary backdrop-blur-3xl hover:bg-primary hover:text-secondary transition-all duration-500">
        {children}
        <BorderBeam
          size={20}
          duration={1}
